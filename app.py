# -*- coding: utf-8 -*-
"""
Created on Tue Jan 31 14:35:50 2023
...
class db:
    def __init__(self):
        ...
        # self.customers['DOB'] = self.customers['DOB'].apply(lambda x: dt.datetime.strptime(x,'%d-%m-%Y'))
        # self.customers['buyers_age'] = (dt.datetime(2023,1,1)-self.customers['DOB'])/np.timedelta64(1,'Y') 
    ...
    def transation_init():
        ...
        transactions['day_of_week'] = transactions['tran_date'].dt.strftime('%A')
        return transactions       
...
app.layout = html.Div([html.Div([dcc.Tabs(id='tabs',value='tab-1',children=[
                            ...
                            dcc.Tab(label='Kanały Sprzedaży')
                            ]),
...
## tab3 callbacks
@app.callback(Output('store_age','figure'),
            [Input('Store_type_dropd','value')])

def tab3_store_age(chosen_cat):
    
    grouped = df.merged[(df.merged['Store_type']==chosen_cat)].pivot_table(index='prod_cat',columns='country',values='total_amt',aggfunc='sum')    
    traces = []
    for col in grouped.columns:
        traces.append(go.Bar(x=grouped[col],y=grouped.index,orientation='h',name=col))
    data = traces
